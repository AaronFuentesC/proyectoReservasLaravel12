<x-layouts::app>
    ...
    <flux:breadcrumbs class="mb-4">
        ...
        <flux:breadcrumbs.item :href="route('admin.bookings.index')">
            Reservas
        </flux:breadcrumbs.item>
        <flux:breadcrumbs.item>
            Editar
    ...
    @php
        $date_start = old('date_start', $booking->start_time->format('d-m-Y')); //Fecha de inicio formateada para poder acceder a ella desde el datepicker
        $time_start = old('time_start', $booking->start_time->format('H:i')); //Hora de inicio formateada para poder acceder a ella desde el timepicker
        $date_end = old('date_end', $booking->end_time->format('d-m-Y')); //Fecha de fin formateada para poder acceder a ella desde el datepicker
        $time_end = old('time_end', $booking->end_time->format('H:i')); //Hora de fin formateada para poder acceder a ella desde el timepicker
    @endphp


    {{-- Se crea un pequeño formulario para editar una reserva. --}}
    <flux:card>
        <form action="{{ route('admin.bookings.update', $booking) }}" method="POST" class="space-y-4"
            enctype="multipart/form-data" x-data="{ sending:false }" @submit="sending=true"> {{-- x-data y @submit se
            van a utilizar para que no se puedan enviar varias peticiones al mismo tiempo --}}
            @csrf
            @method('PUT')
            <flux:input label="Título de la reserva" name="title" value="{{ old('title', $booking->title) }}" {{-- Título
                --}} placeholder="Escribe el título de la reserva"></flux:input>
            <flux:input label="Descripción de la reserva" name="description"
                value="{{ old('description', $booking->description) }}" {{-- Descripción --}}
                placeholder="Escribe la descripción de la reserva" id="description"></flux:input>


            {{-- Div con la fecha y la hora de inicio --}}
                ...
                <label>
                    ...
                    <div class="relative max-w-sm">
                        ...
                        <input datepicker id="default-datepicker-1" type="text"
                            ...
                            placeholder="Select date" name="date_start" value="{{ $date_start }}"
            ...
            {{-- div con la fecha y hora de fin --}}
            <div class="grid grid-cols-2 gap-4">
                <label>
                    Fecha de fin
                    <div class="relative max-w-sm">
                        ...
                        <input datepicker id="default-datepicker-2" type="text"
                            class="block w-full ps-9 pe-3 py-2.5 bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand px-3 py-2.5 shadow-xs placeholder:text-body"
                            placeholder="Select date" name="date_end" value="{{ $date_end}}"
                            datepicker-format="dd-mm-yyyy">
                    </div>
                </label>
                @error('date_end')
            ...
            <livewire:booking-form :booking="$booking" /> {{-- Componente de livewire reactivo con los recursos de la
            reserva --}}

            {{-- Estado de la reserva --}}
            <label>
                Estado de la reserva
                <select class="block w-full border rounded p-2" name="state">
                    @foreach(\App\BookingState::cases() as $state)
                        <option value="{{ $state->value }}" {{ old('state', $booking->state?->value) === $state->value ? 'selected' : '' }}>
                            {{ ucfirst($state->value) }}
                        </option>
                    @endforeach
                </select>
        ...
        {{-- En caso de que ya haya ficheros adjuntos los muestra --}}
